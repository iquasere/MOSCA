...
from mosca_tools import run_command, sort_alphanumeric, parse_blast, run_pipe_command
...
class MetaproteomicsAnalyser:
    ...
    def get_arguments(self):
        ...
        parser.add_argument("-sf", "--spectra-folder", help="Folder with spectra to be analysed")
        parser.add_argument("-fl", "--files-list", help="File listing all input spectra")
        ...
        parser.add_argument("-exps", "--experiment-names", help="Names of experiences (comma-separated)")
        parser.add_argument("--protease", help="Filename in fasta format of protease sequence", default='Trypsin')
        parser.add_argument("-e", "--experiments", help="Experiments file")
        parser.add_argument(
    ...
    def database_generation(
            self, database, output, mpa_result, contaminants_database=None, protease='trypsin',
        ...
        taxids = self.add_reference_proteomes(
            mpa_result, output, references_taxa_level, batch_size=batch_size, max_attempts=max_attempts)
        # Add protease
        if protease == 'Trypsin':
    ...
    def convert_spectra_format(self, files_list, out_dir, format='mgf', peak_picking=False):
        ...
        """
        run_pipe_command(
            f"""msconvert -f {files_list} --{format} -o {out_dir}
    ...
    def peptide_spectrum_matching(
            self, spectra_folder, output, parameters_file, threads='12',
    ...
    def edit_maxquant_mqpar(
            self, mqpar, fasta_database, spectra_folder, experiment_names, threads=1, spectra_format='RAW',
            protein_fdr=0.01):
        """
        input:
        ...
        files = sort_alphanumeric(glob(f'{spectra_folder}/*.{spectra_format}'))
    ...
    def compomics_workflow(
            self, database, output, spectra_folder, threads=1, sample_name='sample', experiment_name='experiment',
            replicate_number='1', protein_fdr=1, max_memory=4096):
        """
        Input:
        ...
        self.peptide_spectrum_matching(
            spectra_folder, output, f'{output}/params.par', threads=threads, max_memory=max_memory)
    ...
    def maxquant_workflow(
            self, mqpar, database, spectra_folder, experiment_names, output, threads=1, spectra_format='RAW',
            protein_fdr=0.01):
        """
        Input:
        ...
        for sample in set(experiments['Sample']):
            ...
            if args.workflow == 'maxquant':
                ...
                for name in set(partial_sample['Name']):
                    if os.path.isfile(
                            '{0}/Metaproteomics/{1}/1st_search/{2}/reports/{1}_{2}_1_Default_Protein_Report_'
                            'with_non-validated_matches.txt'.format(args.output, sample, name)):
                        print(f'[{name}] already analysed!')
                        continue

                    for foldername in ['spectra', '1st_search', '2nd_search']:
                        pathlib.Path(f'{args.output}/Metaproteomics/{sample}/{foldername}/{name}').mkdir(
                    ...
                    for i in range(len(partial_name)):
                        ...
                        if not partial_name.iloc[i]['Files'].endswith('.mgf'):
                            ...
                            shutil.copy(partial_name.iloc[i]['Files'],
                                        f'{args.output}/Metaproteomics/{sample}/spectra/{name}')
                    # Conversion to Mascot Generic Format
                    if len(files2convert) > 0:
                        with open(f'{args.output}/Metaproteomics/{sample}/files2convert.txt', 'w') as f:
                            f.write('\n'.join(files2convert))
                        self.convert_spectra_format(
                            f'{args.output}/Metaproteomics/{sample}/files2convert.txt',
                            f'{args.output}/Metaproteomics/{sample}/spectra/{name}', format='mgf')
                    self.compomics_workflow(
                        f'{args.output}/Metaproteomics/{sample}/1st_search_database.fasta',
                        f'{args.output}/Metaproteomics/{sample}/1st_search/{name}',
                        f'{args.output}/Metaproteomics/{sample}/spectra/{name}',
