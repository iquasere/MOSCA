...
class Reporter:
    ...
    '''
    def write_technical_report(self, tools_list_file, output):                                   # TODO - add proteomics software that cannot be installed with conda
    ...
    def run(self):
        ...
        for i in exps.index:
            self.info_from_preprocessing(args.output, exps.iloc[i]['Name'], exps.iloc[i]['Files'].split(',')[0],
                fastq_columns, performed_rrna_removal = (False if exps.iloc[i]['Data type'] == 'dna' else True))
