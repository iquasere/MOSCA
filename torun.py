...
class Annotater:
    ...
    def recursive_uniprot_information(self, blast, output, max_iter = 20):
        ...
        ids_done = list(result['Entry name'])
        ids = [ide.split('|')[-1] for ide in DIAMOND(out = blast).parse_result()['sseqid']]
        i = 0
        ids_missing_output = '/'.join(output.split('/'))[:-1] 
        while len(set(ids_done)) < len(set(ids)) and i < max_iter:
            print('Information already gathered for ' + str(len(set(ids_done))) + 
                  ' ids. Still missing for ' + str(len(set(ids)) - len(set(ids_done))) + '.')
            print('Checking which IDs are missing information.')
            pbar = ProgressBar()
            ids = [ide for ide in pbar(ids) if ide not in ids_done]
    ...
    '''
    Input: 
        name of an annotated blast file, probably with a lot of not identified (*) proteins (blast)
        name of an InterProScan result file with hopefully CDD annotations (interproscan)
        name of an annotated blast file
    Output: 
        ...
        lines for such protein, since several domains might be identified) (cddblast)
    '''        
    def blast2cddblast(self, blast, interproscan, cddblast, correct_interproscan = True):
        ...
        cddblastdf.to_csv(cddblast, index = False, header = False, sep = '\t')
    ...
    def join_reports(self, blast, uniprotinfo, cog_blast, fun, split_pathways = False,
        ...
        result.columns = result.columns.tolist() + ['Expression']
        ...
        result = pd.merge(result, uniprotinfo, left_index=True, right_on = ['Query'], how = 'outer')
        cog_blast = self.organize_cdd_blast(cog_blast, fun)
        result = pd.merge(result, cog_blast, left_on = ['qseqid'], right_on = ['qseqid'], how = 'outer')
        result = result[['qseqid','Entry name','Taxonomic lineage (SUPERKINGDOM)',
                        ...
                        'COG protein description','Abundance','Expression']]
        result.columns = ['ORF name','UniProt ID','Taxonomic lineage (SUPERKINGDOM)',
                        'Taxonomic lineage (PHYLUM)','Taxonomic lineage (CLASS)',
                        'Taxonomic lineage (ORDER)','Taxonomic lineage (FAMILY)',
                        'Taxonomic lineage (GENUS)','Taxonomic lineage (SPECIES)',
    ...
    def run(self):
        ...
        uniprot_info.to_csv(self.out_dir + '/Annotation/' + self.name + '/uniprot.info', sep = '\t', index = False)
        #self.info_with_coverage(ids, self.out_dir + '/Annotation/' + self.name + '/aligned.blast', self.out_dir, self.assembler)
...
#columns = ['Taxonomic lineage (SUPERKINGDOM)', 'Taxonomic lineage (PHYLUM)', 'Taxonomic lineage (CLASS)', 'Taxonomic lineage (ORDER)', 'Taxonomic lineage (FAMILY)', 'Taxonomic lineage (GENUS)', 'Taxonomic lineage (SPECIES)', 'coverage']
if __name__ == '__main__':
    ...
    for sample in ['4478-DNA-S1618-MiSeqKapa','4478-DNA-S1613-MiSeqKapa','4478-DNA-S1616-MiSeqKapa']:
        print(sample)
        report = annotater.join_reports('MOSCAfinal/Annotation/' + sample + '/aligned.blast',
